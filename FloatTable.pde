...
class FloatTable {
  ...
  String[] columnNames;
  float[] rowTotals;
  ArrayList<String> ignoreRegions;  
  ...
  void calcPercentages(){
    calcTotal();
    percentageData = new float[rowCount][columnCount];
    for(int row = 0; row < rowCount; row++){
      float rmax = getRowTotal(row);
      for(int col = 0; col < columnCount; col++){
        if(isValid(row,col)){
          float val = (data[row][col]/rmax) * 100;
          percentageData[row][col] = val; 
        }
      }
    }
  }
  ...
  void calcTotal(){
  ignoreRegions = new ArrayList<String>();
  ignoreRegions.add("North America");
  ignoreRegions.add("Central & South America");
  ignoreRegions.add("Europe");
  ignoreRegions.add("Eurasia");
  ignoreRegions.add("Middle East");
  ignoreRegions.add("Africa");
  ignoreRegions.add("Asia & Oceania");
  ignoreRegions.add("World");
    rowTotals = new float[rowCount];
    for(int row=0;row<rowCount;row++){
      float rowTotal=0;
      for(int col=0;col<columnCount;col++){
        if(isValid(row,col)){
          if(!ignoreRegions.contains(getColumnName(col))){
